...
Se configuró el enrutamiento estático interno para garantizar el cumplimiento del segmento IP requerido. Los contenedores desplegados y sus direcciones IPv4 asignadas son:

| Servicio / Activo | Contenedor Docker | Dirección IP | Puerto(s) Expuesto(s) | Función |
| :--- | :--- | :--- | :--- | :--- |
| **API Gateway** | `api_gateway_zona4` | `10.2.4.10` | 80 | Balanceador de carga y proxy inverso (NGINX). |
| **Caché** | `redis_zona4` | `10.2.4.20` | 6379 | Servicio de almacenamiento en memoria (Redis). |
| **Message Queue** | `rabbitmq_zona4` | `10.2.4.21` | 5672, 15672 | Servicio de mensajería asíncrona (RabbitMQ). |
| **Aplicación Vulnerable**| `crapi-web` | `10.2.4.30` | 80, 8888 | Frontend principal de crAPI (Completely Ridiculous API). |
...
Como parte de los requerimientos de la fase de Readiness , se implementaron políticas de control de tráfico en el nodo de entrada (API Gateway `10.2.4.10`) editando el archivo `nginx.conf`.

### 3.1. Políticas CORS (Cross-Origin Resource Sharing)
Se establecieron cabeceras HTTP para controlar el acceso a los recursos de la API desde orígenes cruzados:
...
Para mitigar ataques de denegación de servicio (DoS) o fuerza bruta a nivel de aplicación, se configuró una política de limitación de solicitudes:
